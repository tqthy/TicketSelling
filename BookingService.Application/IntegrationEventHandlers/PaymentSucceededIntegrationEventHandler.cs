    ...
    public class PaymentSucceededIntegrationEventHandler : IConsumer<PaymentSucceeded>
    {
        ...
        public async Task Consume(ConsumeContext<PaymentSucceeded> context)
        {
            ...
            try
            {
                ...
                if (booking == null)
                {
                    _logger.LogWarning("Booking {BookingId} not found when processing PaymentSucceeded event. Ignoring message.", message.BookingId);
                ...
                {
                    _logger.LogInformation("Booking {BookingId} is already confirmed. Ignoring duplicate PaymentSucceeded event.", message.BookingId);
                    await transaction.RollbackAsync(context.CancellationToken);
                    return;
                }
                ...
                {
                    _logger.LogWarning("Received PaymentSucceeded for Booking {BookingId} which has status {Status} (expected PendingPayment). Processing cannot continue reliably. Rolling back.",
                 ...
                 if (bookedSeat == null)
                 {
                      _logger.LogError("Inconsistent state: Booking {BookingId} has no BookedSeat associated during PaymentSucceeded processing. Rolling back.", message.BookingId);
                      await transaction.RollbackAsync(context.CancellationToken);
                 ...
                 if (seatStatus == null)
                 {
                      _logger.LogError("Inconsistent state: EventSeatStatus not found for Event {EventId}, Seat {SeatId} associated with confirmed Booking {BookingId}. Rolling back.",
                ...
                if (seatStatus.Status == SeatAvailabilityStatus.Reserved && seatStatus.CurrentBookingId == booking.Id)
                {
                    try
                    {
                        _logger.LogInformation("Marking Seat {SeatId} as Sold for Booking {BookingId}.", seatStatus.SeatId, booking.Id);
                        seatStatus.Sell(booking.Id); // Use domain method
                        seatSoldSuccessfully = true;
                    }
                    catch (InvalidOperationException ex) // Should ideally not happen if the status check above passed, but belt-and-suspenders
                    {
                         _logger.LogError(ex, "Failed to mark Seat {SeatId} as Sold for Booking {BookingId} even though it appeared reserved. Status: {SeatStatus}", seatStatus.SeatId, booking.Id, seatStatus.Status);
                    }
                }
                else
                {
                    ...
                    // *just* before the payment confirmation arrived.
                     _logger.LogWarning("Seat {SeatId} for Event {EventId} was not in the expected 'Reserved' state for Booking {BookingId} when payment succeeded (Current Status: {Status}, Current BookingId: {CurrentBookingId}). Booking will be confirmed, but seat status remains unchanged.",
                ...
                _logger.LogInformation("Successfully processed PaymentSucceeded for Booking {BookingId}. Status Confirmed. Seat Sold: {SeatSold}",
                    message.BookingId, seatSoldSuccessfully);

            }
            catch (Exception ex)
            {
                 _logger.LogError(ex, "Error processing PaymentSucceeded for BookingId: {BookingId}. Rolling back transaction.", message.BookingId);
